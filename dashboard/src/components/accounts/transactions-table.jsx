            ...
            {filtered.map(t => (
              <div key={`mob-tx-${t.type}-${t.id}`} className="rounded-xl border p-3 space-y-2 bg-[#2F264C] border-[#3D3554]">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-xs font-bold text-white">{t.number}</span>
                </div>

                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-white">{t.client_name || t.supplier_name || '—'}</span>
                  <span className="font-bold text-sm" style={{ color: t.type === 'revenue' ? '#10B981' : '#EF4444' }}>
                    {t.type === 'revenue' ? '+' : '-'}{currency} {parseFloat(t.amount).toFixed(2)}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-[#3D3554]/50 text-[11px]">
                  <span className="text-[#A49EC0]">{formatDate(t.date)}</span>
                  <button
                    onClick={() => onViewDetails(t)}
