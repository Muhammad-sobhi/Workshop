...
const paymentMethods = [
  ...
  { key: 'postal_transfer', label: 'حوالة بريدية', icon: Landmark, color: '#ECC796', bg: 'rgba(236,199,150,0.1)', border: 'rgba(236,199,150,0.3)' },
];
...
export default function PaymentDebts({ transactions, paymentMethodFilter, setPaymentMethodFilter, debtsLoading, clientDebts = [], supplierDebts = [], currency, hidePaymentMethods = false, hideDebts = false }) {
  ...
  return (
    <>
      {!hidePaymentMethods && (
        <div className="rounded-2xl border overflow-hidden mb-5" style={{ background: '#201A30', borderColor: '#3D3554' }}>
          <div className="p-4 border-b" style={{ borderColor: '#3D3554' }}>
            ...
            <p className="text-xs mt-0.5" style={{ color: '#A49EC0' }}>صافي الوارد مطروحاً منه الصادر لكل وسيلة سداد نقدية</p>
          </div>
            ...
            {debtsLoading ? (
              ...
              <div key={c.id} className="flex items-center justify-between px-4 py-3 hover:bg-white/5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link href={`/suppliers?tab=clients`} className="px-2.5 py-1 bg-white/10 hover:bg-white/20 transition-all rounded text-[10px] font-bold text-[#ECC796]">
                    كشف الحركة
                  </Link>
                </div>
              </div>
            ...
            {debtsLoading ? (
              ...
              <div key={s.id} className="flex items-center justify-between px-4 py-3 hover:bg-white/5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link href={`/suppliers?tab=suppliers`} className="px-2.5 py-1 bg-white/10 hover:bg-white/20 transition-all rounded text-[10px] font-bold text-[#ECC796]">
                    كشف الحركة
                  </Link>
                </div>
              </div>
