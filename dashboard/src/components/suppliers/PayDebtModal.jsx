...
import { X, DollarSign, Smartphone, Building2, Landmark } from 'lucide-react';
      ...
      <div className="w-full max-w-md rounded-2xl border p-6" style={{ background: '#2F264C', borderColor: '#3D3554' }}>
        <div className="flex items-center justify-between pb-4 border-b mb-4" style={{ borderColor: '#3D3554' }}>
          <div>
            ...
            <p className="text-xs mt-0.5 text-gray-400">{isClient ? 'العميل' : 'المورد'}: {showPayDebt.name} • إجمالي الدين الحالي: {parseFloat(showPayDebt.debt_amount || 0).toFixed(2)} {currency}</p>
          </div>
