...
export default function MaterialForm({ showForm, onClose, form, setForm, editing, saving, msg, onSubmit, categories, units }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={editing ? 'تعديل' : 'إضافة'}>
      <div
        className="w-full max-w-xl rounded-2xl border p-6 max-h-[90vh] overflow-y-auto shadow-2xl transition-all"
        style={{
          background: isLight ? '#FFFFFF' : '#2F264C',
          borderColor: isLight ? '#EBF0FF' : '#3D3554'
        }}
      >
        <div className="flex items-center justify-between pb-4 border-b mb-4" style={{ borderColor: isLight ? '#EBF0FF' : '#3D3554' }}>
          ...
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-black/5" style={{ color: isLight ? '#8288A4' : '#A49EC0' }} aria-label="إغلاق">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="material-code" className="block text-xs font-semibold mb-1.5" style={{ color: isLight ? '#1E293B' : '#D4CEEB' }}>الكود (اتركه فارغاً للتوليد التلقائي)</label>
              <input
                id="material-code"
                value={form.code}
                onChange={e => setForm({ ...form, code: e.target.value })}
                className="w-full rounded-xl px-4 py-2 text-xs border outline-none font-mono font-semibold"
                style={{
                  background: isLight ? '#F5F7FF' : '#231B3D',
                  borderColor: isLight ? '#EBF0FF' : '#3D3554',
                  color: isLight ? '#1E293B' : '#FFFFFF'
            ...
            <div>
              <label htmlFor="material-sku" className="block text-xs font-semibold mb-1.5" style={{ color: isLight ? '#1E293B' : '#D4CEEB' }}>SKU (اتركه فارغاً للتوليد التلقائي)</label>
              <input
                id="material-sku"
                value={form.sku}
          ...
          {form.type === 'material' ? (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="material-initial-stock" className="block text-xs font-semibold mb-1.5" style={{ color: isLight ? '#1E293B' : '#D4CEEB' }}>
                  مخزون أول المدة (الكمية الحالية)
                </label>
                <input
                  id="material-initial-stock"
                  type="number"
                  step="0.01"
                  min="0"
                  ...
                  placeholder="0"
                  className="w-full rounded-xl px-4 py-2 text-xs border outline-none font-mono font-bold"
                  style={{
                    background: isLight ? '#F5F7FF' : '#231B3D',
                    borderColor: isLight ? '#EBF0FF' : '#3D3554',
                    color: isLight ? '#1E293B' : '#FFFFFF'
              ...
              <div>
                <label htmlFor="material-low-stock" className="block text-xs font-semibold mb-1.5" style={{ color: isLight ? '#1E293B' : '#D4CEEB' }}>الحد الأدنى للمخزون (التنبيهات)</label>
                <input
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label htmlFor="material-dimension" className="block text-xs font-semibold mb-1.5" style={{ color: isLight ? '#1E293B' : '#D4CEEB' }}>الأبعاد والمقاسات</label>
              <input
            ...
            <div>
              <label htmlFor="material-unit-cost" className="block text-xs font-semibold mb-1.5" style={{ color: isLight ? '#1E293B' : '#D4CEEB' }}>
                {form.type === 'material' ? 'تكلفة الوحدة' : 'تكلفة الخدمة'} <span className="text-red-500">*</span>
              </label>
              <input
          ...
          <div>
            <label htmlFor="material-description" className="block text-xs font-semibold mb-1.5" style={{ color: isLight ? '#1E293B' : '#D4CEEB' }}>الوصف وتفاصيل التكلفة</label>
            <textarea
              id="material-description"
              value={form.description}
              onChange={e => setForm({ ...form, description: e.target.value })}
              rows={2}
              className="w-full rounded-xl px-4 py-2 text-xs border outline-none resize-none font-medium"
              style={{
                background: isLight ? '#F5F7FF' : '#231B3D',
                borderColor: isLight ? '#EBF0FF' : '#3D3554',
                color: isLight ? '#1E293B' : '#FFFFFF'
          ...
          {msg && (
            <p className={`text-xs text-center py-2.5 rounded-xl font-semibold ${msg.includes('نجاح') || msg.includes('تم') ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'}`}>
              {msg}
            </p>
          )}
          ...
          <div className="flex gap-3 pt-2">
            <button
              type="submit"
              disabled={saving}
              className="flex-1 py-2.5 rounded-xl font-bold text-xs shadow-md transition-all text-white hover:opacity-90"
              style={{
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-2.5 rounded-xl font-bold text-xs border"
              style={{
                borderColor: isLight ? '#EBF0FF' : '#3D3554',
